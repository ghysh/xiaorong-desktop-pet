"""Fail-fast Windows build pipeline for the minimal XiaoRong 1.2.0 release."""
...
from desktop_pet.paths import (
    ...
    DROWSY_SLEEP_FRAMES_DIR,
    DROWSY_SLEEP_MANIFEST,
...
from generate_release_checksums import sha256_file
from verify_xiaorong_release import verify_executable

VERSION_TAG = __version__.replace(".", "_")
BUILD_DIRECTORY_NAME = f"xiaorong_{VERSION_TAG}"
BUILD_ROOT = PROJECT_ROOT / "build" / "pyinstaller" / BUILD_DIRECTORY_NAME
DIST_ROOT = PROJECT_ROOT / "dist" / "pyinstaller" / BUILD_DIRECTORY_NAME
REPORTS_DIR = PROJECT_ROOT / "build" / "reports"
RELEASE_DIR = PROJECT_ROOT / "release"
SPEC_PATH = PROJECT_ROOT / "packaging" / "windows" / "xiaorong.spec"
FINAL_EXECUTABLE_NAME = f"小融-{__version__}-win64.exe"
EXPECTED_PYTHON = (3, 11, 15)
DROWSY_SLEEP_FRAME_NAMES = tuple(
    sorted(
        {
            Path(frame["asset_path"]).name
            for frame in json.loads(DROWSY_SLEEP_MANIFEST.read_text(encoding="utf-8"))["frames"]
        }
    )
)
DROWSY_SLEEP_RESOURCES = ((DROWSY_SLEEP_MANIFEST, "drowsy sleep manifest"),) + tuple(
    (DROWSY_SLEEP_FRAMES_DIR / name, f"drowsy sleep frame {name}")
    for name in DROWSY_SLEEP_FRAME_NAMES
)
RUNTIME_RESOURCES = DROWSY_SLEEP_RESOURCES + (
    (FULLBODY_RUNTIME_MASTER, "正式角色主图"),
    (CLICK_DIALOGUE_FILE, "当前点击对白"),
    (DIALOGUE_BUBBLE_FRAME, "点击对白气泡框"),
...
def run_optional_git_diff_check() -> dict[str, object]:
    """Run diff validation when Git metadata is visible to the build identity."""
    ...
    if "not a git repository" in output.casefold():
        announce("构建身份无法访问 .git；diff check 由外层工作区验证，打包继续。")
        return {"status": "skipped_git_metadata_unavailable", "output": output}
    raise RuntimeError(f"git diff --check failed with exit code {completed.returncode}")


def assert_build_environment() -> dict[str, str]:
    if platform.system() != "Windows" or platform.machine().casefold() not in {"amd64", "x86_64"}:
        raise RuntimeError(f"小融 {__version__} 只能在 Windows x64 构建。")
    ...
    if __version__ != "1.2.0" or WINDOWS_FILE_VERSION != (1, 2, 0, 0):
        raise RuntimeError("应用版本元数据不是 1.2.0。")
    return {
...
def inspect_protected_inputs() -> dict[str, object]:
    ...
    with Image.open(BLINK_FRAMES_DIR / "blink_open.png") as frame:
        frame.verify()
    with Image.open(DIALOGUE_BUBBLE_FRAME) as bubble_frame:
        if bubble_frame.format != "PNG" or "A" not in bubble_frame.getbands():
            raise ValueError(f"对白气泡框必须是透明 PNG：{DIALOGUE_BUBBLE_FRAME}")
        bubble_frame.verify()
    ...
    return {
        ...
        "dialogue_bubble_frame": {
            "path": str(DIALOGUE_BUBBLE_FRAME),
            "sha256": sha256_file(DIALOGUE_BUBBLE_FRAME),
            "size": DIALOGUE_BUBBLE_FRAME.stat().st_size,
            "modified_ns": DIALOGUE_BUBBLE_FRAME.stat().st_mtime_ns,
        },
        "icon": {
...
def safe_clean_version_directory(path: Path, allowed_parent: Path) -> None:
    ...
    if resolved_path.parent != resolved_parent or resolved_path.name != BUILD_DIRECTORY_NAME:
...
def prepare_release_directory() -> Path | None:
    """Archive an existing release directory before creating the one-file output."""
    resolved_release = RELEASE_DIR.resolve()
    if resolved_release.parent != PROJECT_ROOT.resolve() or resolved_release.name != "release":
        raise RuntimeError(f"拒绝处理未经验证的发布目录：{resolved_release}")
    archived: Path | None = None
    if RELEASE_DIR.exists() and any(RELEASE_DIR.iterdir()):
        archive_name = datetime.now().strftime("release_archive_%Y%m%d_%H%M%S")
        archived = PROJECT_ROOT / archive_name
        if archived.exists():
            raise FileExistsError(f"发布归档目标已存在：{archived}")
        announce(f"归档现有 release 目录：{archived}")
        try:
            RELEASE_DIR.replace(archived)
        except PermissionError:
            announce("Python 目录重命名被历史 ACL 拒绝，改用 Windows PowerShell 原生移动。")
            run_command(
                [
                    "powershell.exe",
                    "-NoProfile",
                    "-Command",
                    "Move-Item -LiteralPath $args[0] -Destination $args[1] -ErrorAction Stop",
                    str(RELEASE_DIR),
                    str(archived),
                ]
            )
            if RELEASE_DIR.exists() or not archived.is_dir():
                raise RuntimeError("PowerShell 未能完成旧 release 目录归档。")
...
def verify_double_launch(executable: Path) -> dict[str, object]:
    ...
    return {
        ...
        "known_limitation": f"连续启动会创建两个独立实例；{__version__} 未新增单实例机制。",
    }
...
def main() -> int:
    ...
    announce("执行环境检查、发布专项 pytest、完整 pytest、Ruff、diff check 和运行依赖审计")
    environment_output = run_command([sys.executable, "scripts/check_environment.py"])
    targeted_pytest_output = run_command(
        [
            sys.executable,
            "-m",
            "pytest",
            ...
            "tests/test_xiaorong_release_config.py",
            "tests/test_packaged_resource_layout.py",
            "tests/test_drowsy_sleep.py",
            "tests/test_drowsy_sleep_menu.py",
            "tests/test_stage10b_runtime_gate.py",
            "-q",
            "--basetemp",
            str(BUILD_ROOT / "pytest_targeted"),
        ]
    )
    ...
    git_diff_check = run_optional_git_diff_check()
    run_command([sys.executable, "scripts/audit_runtime_imports.py"])
    import_audit_path = REPORTS_DIR / f"runtime_import_audit_{VERSION_TAG}.json"
    ...
    write_report(REPORTS_DIR / f"xiaorong_onedir_verification_{VERSION_TAG}.json", onedir_report)
    if not onedir_report["passed"]:
        raise RuntimeError("onedir 本地验证失败。")
    clean_onedir_report = verify_clean_onedir(onedir_executable)
    write_report(REPORTS_DIR / f"xiaorong_onedir_clean_path_{VERSION_TAG}.json", clean_onedir_report)
    ...
    write_report(REPORTS_DIR / f"xiaorong_onefile_verification_{VERSION_TAG}.json", onefile_report)
    ...
    write_report(REPORTS_DIR / f"xiaorong_onefile_clean_path_{VERSION_TAG}.json", clean_onefile_report)
    ...
    archived_release = prepare_release_directory()
    ...
    write_report(REPORTS_DIR / f"xiaorong_final_release_verification_{VERSION_TAG}.json", final_report)
    ...
    write_report(REPORTS_DIR / f"release_contents_audit_{VERSION_TAG}.json", final_audit)
    ...
    report = {
        ...
        "targeted_pytest_output": targeted_pytest_output,
        ...
        "archived_previous_release": None if archived_release is None else str(archived_release),
        "windows_sandbox_tested": False,
        "digital_signature": False,
        "git_commit_created": False,
        "release_uploaded": False,
    }
    report_path = REPORTS_DIR / f"release_build_{__version__}.json"
    write_report(report_path, report)
    announce(f"小融 {__version__} 发布构建与验证完成：{final_executable}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"小融 {__version__} 构建失败（{type(error).__name__}）：{error}", file=sys.stderr, flush=True)
